# Hi there, I'm Gary Chew 👋 (Splunk Full Stack Observability Architect)

I am a passionate technologist with customer focus, with experience across a range of infrastructure technologies, platforms and services. 

I am currently a Observability Architect at Splunk, supporting customers through out APAC, based out of Singapore.

My main areas of expertise:
- **Full-stack Observability (Kubernetes and Container Monitoring, AI Agent Observability, OpenTelemetry, Application Performance Monitoring, Database Visibility, Infrastructure Visibility, Browser Real User Monitoring, Mobile Real User Monitoring, Synthetic Monitoring, Application Analytics, Splunk, Splunk Observability Cloud and AppDynamics)**.
- **DevOps Toolings such as Kubernetes, Docker, Jenkins, Ansible, ArgoCD, etc**.
- **Cloud Transformation, Cloud Security, Cloud Architecture, Infrastructure as Code**. 
  
---
### 💼 Professional Summary
- 🔭 I’m currently working on **Helping customer to implement Full Stack Observability**
- 🌱 I’m currently learning **Splunk Agent Observability & Agentic AI**
- 💬 Talk to me about **Observability, OpenTelemetry, Kubernetes, Devops, System Design**

---
### 🛠️ Skills & Technologies

**Languages & Core Tech:**
`Full Stack Observability` `Monitors, evaluates, and secures autonomous AI agents and large language models (LLMs) in production` `Java` `Oracle`

**Frameworks & Tools:**
`Splunk Observability Cloud` `AppDynamics` `Kubernetes` `Spring Boot` `AWS` `VS Copilot` `Antigravity` `Xcode` `Android Studio`

---

### 🤝 Connect with Me
Let's network! You can find me on these platforms:

[![LinkedIn](https://shields.io)](https://www.linkedin.com/in/gary-chew-145a711/)
[![Hotmail](https://shields.io)](mailto:neechuan@hotmail.com)
